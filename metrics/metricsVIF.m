% The code is provided by the original author of this metric
% The interface is modified by the author of MEFB to integrate it into MEFB. 

%
% Reference for the metric:
% Y. Han, Y. Cai, Y. Cao, X. Xu, A new image fusion performance metric based on visual information fidelity, Inf. Fusion 14 (2) (2013) 127每135.

function res=metricsVIF(img1,img2,fused)
    ...
    if b == 1
        g = VIF(img1,img2,fused);
        res = g;
    elseif b1 == 1
        for k = 1 : b 
           g(k) = VIF(img1(:,:,k),img2,fused(:,:,k)); 
        end 
        res = mean(g); 
    else
        for k = 1 : b 
            g(k) = VIF(img1(:,:,k),img2(:,:,k),fused(:,:,k)); 
        end 
        res = mean(g); 
    end
end
...
%   	based on visual information fidelity, information fusion, Volume 14, Issue 2, April 2013, Pages 127每135
...
function output = VIF (Im1,Im2,ImF)
% Cite this work as
% []Yu Han, Yunze Cai, Yin Cao, Xiaoming Xu, A new image fusion performance metric based on visual information fidelity, information fusion, Volume 14, Issue 2, April 2013, Pages 127每135
% input:
%       Im1, source image 1
%       Im2, source image 2
%       ImF, fused image
...
function [Tg1,Tg2,Tg3]=ComVidVindG(ref,dist,sq)
     ...
     % [] H.R.Sheikh and A.C.Bovik, Image information and visual quality[J], IEEE Transactions on Image Processing 15(2), pp. 430每444, 2006.
